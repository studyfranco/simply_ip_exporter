...
/// APIs and literal patterns that mean "this line executes raw SQL", per `AGENT.MD`: "Never write
/// vendor-specific raw SQL queries. Zero raw SQL for DML in `src/`." `src/db.rs` is the documented
/// exception — session pragmas are not DML, and there is nowhere else pragmas could live given
/// SeaORM has no query-builder API for them.
...
#[test]
fn no_raw_sql_outside_db_rs_and_migrations() {
    let mut violations = Vec::new();
    for path in src_files() {
        if path == "src/db.rs" || path.starts_with("src/migration/") {
            continue;
        }
    ...
    assert!(
        violations.is_empty(),
        "raw SQL found outside src/db.rs and src/migration/ (AGENT.MD forbids this):\n{}",
        violations.join("\n")
    );
}
